...
class ConfiguracionService:
    ...
    # ------------------------------------------------------------------
    # CRUD genérico de catálogos socioeconómicos
    # ------------------------------------------------------------------

    @staticmethod
    @transaction.atomic
    def crear_entrada_catalogo(modelo, form_data: dict):
        """Crea una nueva entrada en el catálogo dado.

        Args:
            modelo: Clase del modelo catálogo.
            form_data: Dict con los datos validados del form.

        Returns:
            La instancia creada.

        Raises:
            ValueError: Si el nombre ya existe en el catálogo.
        """
        try:
            return modelo.objects.create(**form_data)
        except IntegrityError:
            raise ValueError("Ya existe una opción con ese nombre en este catálogo.")

    @staticmethod
    @transaction.atomic
    def editar_entrada_catalogo(instancia, form_data: dict):
        """Actualiza una entrada existente del catálogo.

        Args:
            instancia: Objeto del modelo a actualizar.
            form_data: Dict con los datos validados del form.
        ...
        Raises:
            ValueError: Si el nombre ya existe en otra entrada del catálogo.
        """
        try:
    ...
    @staticmethod
    @transaction.atomic
    def toggle_activo_catalogo(instancia):
        """Invierte el estado activo/inactivo de una entrada del catálogo.
    ...
    @staticmethod
    @transaction.atomic
    def eliminar_entrada_catalogo(instancia):
        """Elimina una entrada del catálogo.

        Args:
            instancia: Objeto del modelo a eliminar.
        """
        instancia.delete()
